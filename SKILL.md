...
**执行任务**：分析题目，确定数学模型，产出分析文档。建模手技能在此作为**审查角色**运行——对建模交付物做逻辑闭环、编程可行性、公式变量、数据可行性与术语一致性审查，判断是否可进入代码实现（不再由建模手规定完整建模流程）。

执行步骤（由你/编程手进行建模分析，并加载建模手技能对其交付物做审查）：

- 阅读并理解题目要求
- 为每道题确定合适的数学模型和算法
- 记录模型对应的公式、论文引用等信息
- 创建术语表格保持术语一致性
- 将题目拆分为可独立实现的子问题，建立子问题-模型映射表（含算法类别、数据规模、所需库等路由客观输入）
- 语言路由不由建模手产出；路由客观输入宜写入交付物（非强制），供编程手在编程手阶段按 `建模手/references/语言路由指南.md` 客观判定并确认（未提供则编程手独立推导）
...
- 依据建模手交付物中的「路由客观输入」自行建立语言路由表（Python / MATLAB），按子问题向用户确认
- 对每个子问题先向用户展示语言推荐、客观依据、备选语言和执行计划；未获确认前不得写代码、运行代码或进入下一个子问题
- 按每道题目分开编写结构清晰的代码
...
3. **术语一致性**：审核建模手阶段创建的术语表格应被后续阶段使用
