...
% 预测
predict_steps = 3;
total_steps = n + predict_steps;
predictions = zeros(1, total_steps);
predictions(1) = data(1);
for k = 2:total_steps
    predictions(k) = (data(1) - b/a) * (1 - exp(a)) * exp(-a * (k-1));
end

% 拟合值与残差
fitted = predictions(1:n);
residuals = data - fitted;

% 模型评估
relative_errors = abs(residuals) ./ data * 100;
mean_relative_error = mean(relative_errors);
std_original = std(data, 1);
std_residual = std(residuals, 1);
C = std_residual / std_original;
S0 = 0.6745 * std_original;
small_error_count = sum(abs(residuals - mean(residuals)) < S0);
P = small_error_count / n;
...
y = [2.1, 4.0, 6.2, 7.8, 10.5, 12.1, 14.0, 16.2, 18.1, 20.0]';
...
rng(42);
X = randn(100, 3);
y = 3*X(:, 1) + 2*X(:, 2) - X(:, 3) + 5 + randn(100, 1) * 0.5;

mdl_multi = fitlm(X, y);
disp(mdl_multi);

fprintf('回归方程: y = %.4f*x1 + %.4f*x2 + %.4f*x3 + %.4f\n', ...
    mdl_multi.Coefficients.Estimate(2:4), mdl_multi.Coefficients.Estimate(1));
...
%% ARIMA时间序列预测
% 示例数据
data = [2.874, 3.278, 3.337, 3.390, 3.679, 3.850, 3.920, 4.010, 4.150, 4.280];
train_data = data(1:7);
test_data = data(8:end);
...
% 预测
forecast_steps = 3;
[forecast, ~] = forecast(fitted_model, forecast_steps, 'Y', train_data');

fprintf('ARIMA(1,1,1)预测值:\n');
...
for p = 0:max_p
    for d = 0:max_d
        for q = 0:max_q
            ...
            try
                model_temp = arima(p, d, q);
                [~, ~, logL] = estimate(model_temp, train_data', 'Display', 'off');
                [aic, ~] = aicbic(logL, p+q+1, length(train_data));
                if aic < best_aic
                    best_aic = aic;
                    best_order = [p, d, q];
                end
...
```matlab
%% Holt-Winters指数平滑法手动实现
% 示例数据：带季节性的季度销量
data = [120, 135, 110, 145, 130, 150, 125, 155, 140, 160, 135, 165, ...
        125, 140, 115, 150, 135, 155, 130, 160, 145, 165, 140, 170];
seasonal_periods = 4;
n = length(data);
...
% 初始季节因子
for j = 1:seasonal_periods
    S(j) = data(j) / mean(data(1:seasonal_periods));
end

% 初始水平和趋势
L(seasonal_periods) = mean(data(1:seasonal_periods));
T(seasonal_periods) = (mean(data(seasonal_periods+1:2*seasonal_periods)) - ...
                       mean(data(1:seasonal_periods))) / seasonal_periods;

% Holt-Winters迭代更新
for t = seasonal_periods+1:n
    L(t) = alpha * data(t) / S(t-seasonal_periods) + (1-alpha) * (L(t-1) + T(t-1));
    T(t) = beta * (L(t) - L(t-1)) + (1-beta) * T(t-1);
    S(t) = gamma * data(t) / L(t) + (1-gamma) * S(t-seasonal_periods);
end

% 预测未来4期
predict_steps = 4;
forecast = zeros(1, predict_steps);
for m = 1:predict_steps
    idx = mod(n + m - 1, seasonal_periods) + 1;
    forecast(m) = (L(n) + m * T(n)) * S(idx);
end

fprintf('Holt-Winters预测结果:\n');
for i = 1:predict_steps
    fprintf('未来第%d期: %.2f\n', i, forecast(i));
end
```
...
| 算法 | 时间复杂度 | 空间复杂度 | 说明 |
|------|-----------|-----------|------|
| 灰色预测 GM(1,1) | O(n²) | O(n) | n 为序列长度 |
| 插值拟合 | O(n³) 最坏 | O(n²) | 多项式插值高次不稳定 |
| 线性回归 | O(n·d² + d³) | O(d²) | d 为特征数 |
| 神经网络 | O(E·B·L·d²) | O(L·d²) | E 为 epoch，B 为 batch，L 为层数 |
| SVM | O(n²·d) 最坏 | O(n²) | 核矩阵占用大 |
| ARIMA | O(n²) | O(n) | 模型阶数通常较小 |
| 指数平滑 | O(n) | O(n) | 线性复杂度 |
| Prophet | O(n·k) | O(n) | k 为变点数量 |
| XGBoost | O(T·n·d·log n) | O(T·n) | T 为树数量 |
| 时空预测（STGCN） | O(E·B·T·n·d) | O(n²) | 图卷积计算开销大 |

注：复杂度由算法本身决定，Python 与 MATLAB 实现共享同一复杂度。

### 局限性

- **灰色预测仅适合短期单调序列**：GM(1,1) 模型对具有指数增长趋势的短序列效果较好，但中长期预测误差迅速累积，且无法处理周期性波动或负值数据。
- **神经网络需大量数据**：深度学习模型（LSTM、CNN）需要大量训练样本才能发挥优势，小样本场景下容易过拟合，且可解释性差。
- **ARIMA 要求序列平稳性**：ARIMA 模型要求时间序列满足平稳性假设，非平稳序列需先差分处理；同时对季节性波动和异常值敏感。
- **SVM 核函数选择困难**：SVM 的预测性能高度依赖核函数和超参数的选择，不恰当的核函数会导致模型欠拟合或过拟合。
- **XGBoost 参数空间大**：树深度、学习率、子采样率等超参数众多，调参耗时，且容易在小数据集上过拟合。
- **Prophet 对突变点敏感**：变点检测受先验参数影响较大，数据量过小时变点位置估计不稳定。

### 比赛使用建议

- **数据量小（<30 样本）优先用灰色预测**：灰色预测适合"贫信息"系统，在小样本场景下效果优于统计方法。建议配合后验差检验和残差检验评估模型精度。
- **有时间序列特征优先用 ARIMA 或 Prophet**：ARIMA 适合短中期平稳序列，Prophet 适合含节假日效应的长序列。建议用 ACF/PACF 图辅助定阶，用 AIC/BIC 选择最优模型。
- **分类预测任务优先用 SVM 或 XGBoost**：SVM 在小样本分类中表现优异，XGBoost 在结构化数据预测中通用性强。建议用网格搜索 + 交叉验证调参。
- **多模型对比与集成**：至少使用两种不同原理的预测方法（如统计模型 + 机器学习），对比 RMSE、MAE、MAPE 等指标，必要时用 Stacking 集成提升精度。
- **残差分析不可少**：无论使用哪种模型，必须对残差进行白噪声检验（Ljung-Box 检验），判断模型是否充分提取了数据中的信息。
