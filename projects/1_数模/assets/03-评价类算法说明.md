...
lambda_max = max(diag(eig_val));
[~, max_idx] = max(diag(eig_val));
w = abs(eig_vec(:, max_idx));
w = w / sum(w);
...
col_sums = sum(scores, 1);
normalized_scores = scores ./ col_sums;
total_scores = normalized_scores * w;

fprintf('\n综合得分:\n');
for i = 1:length(destinations)
    fprintf('  %s: %.4f\n', destinations{i}, total_scores(i));
end
```
...
% 向量标准化
normalized = data ./ vecnorm(data, 2, 1);

% 加权规范化
weighted = normalized .* weights;

% 确定正负理想解
v_positive = zeros(1, n);
v_negative = zeros(1, n);
for j = 1:n
    if directions(j) == 1
        v_positive(j) = max(weighted(:, j));
        v_negative(j) = min(weighted(:, j));
    else
        v_positive(j) = min(weighted(:, j));
        v_negative(j) = max(weighted(:, j));
    end
end

% 计算欧氏距离
d_positive = sqrt(sum((weighted - v_positive).^2, 2));
d_negative = sqrt(sum((weighted - v_negative).^2, 2));
...
fprintf('TOPSIS分析结果:\n');
for i = 1:m
    fprintf('  %s: C = %.4f\n', suppliers{i}, closeness(i));
end

fprintf('\n推荐排序:\n');
for i = 1:m
    fprintf('  第%d名: %s (C = %.4f)\n', i, suppliers{ranking(i)}, closeness(ranking(i)));
end
```
...
```matlab
%% DEA数据包络分析（CCR模型，投入导向）
% 示例数据：5个DMU，2个投入，2个产出
inputs = [2, 3; 3, 2; 4, 1; 2, 2; 3, 3];
outputs = [3, 2; 4, 3; 5, 2; 3, 4; 4, 3];
[n_dmus, n_inputs] = size(inputs);
n_outputs = size(outputs, 2);

efficiencies = zeros(n_dmus, 1);
...
for k = 1:n_dmus
    ...
    % 不等式约束 A_ub * x <= b_ub
    A_ub = zeros(n_inputs + n_outputs, 1 + n_dmus);
    b_ub = zeros(n_inputs + n_outputs, 1);

    % 投入约束: sum(lambda_j * x_ij) <= theta * x_i0
    for i = 1:n_inputs
        A_ub(i, 1) = x0(i);
        A_ub(i, 2:end) = inputs(:, i)';
        b_ub(i) = 0;
    end

    % 产出约束: sum(lambda_j * y_rj) >= y_r0
    for r = 1:n_outputs
        A_ub(n_inputs + r, 2:end) = -outputs(:, r)';
        b_ub(n_inputs + r) = -y0(r);
    end

    lb = [-inf; zeros(n_dmus, 1)];
    options = optimoptions('linprog', 'Display', 'off');
    [sol, ~, exitflag] = linprog(f, A_ub, b_ub, [], [], lb, [], options);

    if exitflag > 0
        efficiencies(k) = sol(1);
    else
        efficiencies(k) = NaN;
    end
end

dmu_names = {'DMU1', 'DMU2', 'DMU3', 'DMU4', 'DMU5'};
fprintf('DEA效率值:\n');
for i = 1:n_dmus
    fprintf('  %s: %.4f\n', dmu_names{i}, efficiencies(i));
end
```
...
| 算法 | 时间复杂度 | 空间复杂度 | 说明 |
|------|-----------|-----------|------|
| 层次分析法 AHP | O(n²) | O(n²) | n 为指标数，判断矩阵计算 |
| 模糊层次分析 Fuzzy-AHP | O(n²) | O(n²) | 含模糊数运算额外开销 |
| 熵权法 | O(n·m) | O(n·m) | n 为指标数，m 为方案数 |
| TOPSIS | O(n·m) | O(n·m) | 正负理想解计算 |
| 灰色关联分析 GRA | O(n·m) | O(n·m) | 关联系数矩阵计算 |
| 秩和比法 RSR | O(n·m) | O(n·m) | 编秩计算 |
| 变异系数法 | O(n·m) | O(n·m) | 均值和标准差计算 |
| 主观赋权法（Delphi） | O(k·n²) | O(n²) | k 为专家轮数 |
| 数据包络分析 DEA | O(m·n²) | O(n²) | 线性规划求解次数 |
| 区间数评价 | O(n²·m) | O(n²) | 区间数运算复杂 |

注：复杂度由算法本身决定，Python 与 MATLAB 实现共享同一复杂度。

### 局限性

- **AHP 主观性强**：判断矩阵由专家打分构造，带有较强主观性，不同专家可能给出差异较大的结果。一致性检验通过并不保证判断合理。
- **TOPSIS 对权重敏感**：评价结果高度依赖指标权重的选择，权重微小变化可能导致排序结果大幅变动，需结合多种赋权方法验证。
- **DEA 对 DMU 数量有要求**：决策单元（DMU）数量至少为输入输出指标数量的 2-3 倍，否则区分度不足；且 DEA 对异常值敏感。
- **熵权法忽略指标间相关性**：纯基于数据离散程度赋权，可能赋予高度相关指标重复权重，导致评价偏差。
- **灰色关联分析分辨系数选取主观**：分辨系数 ρ 通常取 0.5，但不同取值对关联度排序可能产生影响，缺乏客观选取标准。
- **区间数评价结果解释困难**：区间数运算得到的评价结果仍为区间，方案排序需额外比较规则，结果不直观。

### 比赛使用建议

- **主客观结合赋权是标配**：建议同时使用主观赋权法（AHP）和客观赋权法（熵权法、变异系数法），取加权综合或对比展示两种权重下的排序结果。
- **多种方法交叉验证**：选用至少 2-3 种不同原理的评价方法（如 TOPSIS、GRA、RSR）对同一方案集合排序，用 Kendall 协调系数检验排序一致性，增强结论可信度。
- **灵敏度分析不可少**：对权重变化进行灵敏度分析，展示权重在 ±10%、±20% 范围内波动时排序结果的变化，证明结论的稳健性。
- **DEA 需注意 DMU 数量**：使用 DEA 时确保 DMU 数量充足，必要时合并指标维度或使用超效率 DEA 增强区分度。
- **指标标准化方法需说明**：不同标准化方法（极差归一化、Z-score、向量归一化）对结果有显著影响，应在论文中说明选取依据并做敏感性分析。
