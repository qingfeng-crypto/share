...
values = f(samples);
estimate = (b - a) * mean(values);
std_error = (b - a) * std(values) / sqrt(n);
ci = [estimate - 1.96 * std_error, estimate + 1.96 * std_error];
...
%% 收敛曲线
cumulative_mean = cumsum(rand(n, 1)) ./ (1:n)';
figure;
plot(1:n, cumulative_mean);
xlabel('采样次数'); ylabel('估计值');
title('蒙特卡洛收敛曲线');
grid on;
```
...
if rho >= 1
    error('系统不稳定，需要lambda < mu');
end

L = rho / (1 - rho);                % 平均队长
Lq = rho^2 / (1 - rho);             % 平均等待队长
W = 1 / (mu - lambda);              % 平均逗留时间
Wq = lambda / (mu * (mu - lambda)); % 平均等待时间
p0 = 1 - rho;                       % 系统空闲概率
...
sum_term = 0;
for k = 0:c-1
    sum_term = sum_term + (c * rho_c)^k / factorial(k);
end
p0_c = 1 / (sum_term + (c * rho_c)^c / (factorial(c) * (1 - rho_c)));

% 性能指标
Lq_c = p0_c * (c * rho_c)^c * rho_c / (factorial(c) * (1 - rho_c)^2);
L_c = Lq_c + c * rho_c;
Wq_c = Lq_c / lambda;
W_c = L_c / lambda;
...
Ilachinski | 2001 | World Scientific |
...
```matlab
%% 马尔科夫链演化
P = [0.7, 0.3; 0.4, 0.6];  % 转移概率矩阵
pi0 = [1, 0];               % 初始分布
n_steps = 20;

pi_history = zeros(n_steps + 1, 2);
pi_history(1, :) = pi0;
...
disp('各步概率分布:');
for t = 0:n_steps
    fprintf('t=%d: [%.4f, %.4f]\n', t, pi_history(t+1, 1), pi_history(t+1, 2));
end
...
figure;
t_vec = 0:n_steps;
plot(t_vec, pi_history(:, 1), 'b-o', t_vec, pi_history(:, 2), 'r-s');
...
% SIR模型
beta = 0.3; gamma = 0.1;
sir_ode = @(t, y) [-beta * y(1) * y(2) / (y(1)+y(2)+y(3));
                    beta * y(1) * y(2) / (y(1)+y(2)+y(3)) - gamma * y(2);
                    gamma * y(2)];
y0 = [990, 10, 0];
tspan = [0, 160];
...
%% Lotka-Volterra模型
alpha = 1.5; beta_lv = 1.0; delta = 1.0; gamma_lv = 1.5;
lv_ode = @(t, y) [alpha * y(1) - beta_lv * y(1) * y(2);
                   delta * y(1) * y(2) - gamma_lv * y(2)];
y0_lv = [10, 5];
[t_lv, y_lv] = ode45(lv_ode, [0, 20], y0_lv);

figure;
subplot(1,2,1);
plot(t_lv, y_lv(:,1), 'b-', t_lv, y_lv(:,2), 'r-', 'LineWidth', 1.5);
xlabel('Time'); ylabel('Population');
...
subplot(1,2,2);
plot(y_lv(:,1), y_lv(:,2), 'k-', 'LineWidth', 1.5);
xlabel('Prey'); ylabel('Predator');
title('Phase Portrait');
grid on;

%% 偏微分方程求解 - pdepe
% 热传导方程: du/dt = d^2u/dx^2, u(0,t)=0, u(1,t)=0, u(x,0)=sin(pi*x)

m = 0;
pdefun = @(x, t, u, DuDx) deal(1, DuDx);
icfun = @(x) sin(pi * x);
...
| 算法 | 时间复杂度 | 空间复杂度 | 说明 |
|------|-----------|-----------|------|
| 蒙特卡洛模拟 | O(N·f) | O(N) | N 为采样次数，f 为单次评估 |
| 排队论（M/M/1 解析解） | O(1) | O(1) | 解析公式直接计算 |
| 排队论（M/M/c 数值解） | O(c) | O(c) | 迭代求稳态概率 |
| 排队论（仿真） | O(N·事件数) | O(队列长度) | 事件驱动仿真 |
| 博弈论（纳什均衡） | O(n²·m) | O(n·m) | n 为策略数，m 为玩家数 |
| 元胞自动机 | O(G·N·r) | O(N) | G 为时间步，N 为元胞数，r 为邻域半径 |
| 马尔科夫链（稳态分布） | O(n³) | O(n²) | 求解线性方程组 |
| 马尔科夫链（仿真） | O(N·n) | O(n) | N 为模拟步数 |
| 微分方程（ODE 数值解） | O(N·n²) | O(n²) | N 为步数，n 为方程维数 |
| 微分方程（PDE 数值解） | O(N·nx·ny·nz) | O(nx·ny·nz) | 空间网格离散化 |

注：复杂度由算法本身决定，Python 与 MATLAB 实现共享同一复杂度。

### 局限性

- **蒙特卡洛收敛速度慢**：误差收敛速度为 O(1/√N)，要达到高精度需要大量采样，计算成本高。方差缩减技术（重要抽样、对偶变量等）可缓解但无法根本解决。
- **元胞自动机规则设计困难**：CA 的演化规则需要人工设计，规则不合理时模拟结果与实际偏差大。复杂系统难以找到简单规则有效模拟。
- **ODE 数值稳定性受步长影响**：显式方法（如 Euler、RK4）的稳定性受步长限制，步长过大可能导致数值发散；刚性问题需使用隐式求解器或变步长方法。
- **排队论解析解适用范围有限**：M/M/1 等经典排队模型的解析解需要严格的假设（泊松到达、指数服务时间），实际系统往往不满足这些假设，只能通过仿真求解。
- **博弈论完全理性假设不现实**：经典博弈论假设参与人完全理性，实际决策中有限理性、信息不完全、学习效应等因素导致纳什均衡预测不准。
- **马尔科夫链无记忆性限制**：马尔科夫链要求未来状态只与当前状态有关（无记忆性），实际问题中往往存在高阶依赖或长期记忆效应。

### 比赛使用建议

- **蒙特卡洛配合方差缩减**：使用重要抽样、对偶变量、控制变量等方差缩减技术，在相同采样次数下提高精度，并在论文中对比缩减前后的误差。
- **ODE 用变步长求解器**：优先使用 MATLAB 的 `ode45`（RK4 自适应步长）或 `ode15s`（刚性问题），避免固定步长显式方法。在论文中展示数值解的稳定性验证。
- **排队论先试解析解再仿真**：先尝试排队论解析公式求解，若假设不满足则改用仿真。仿真结果需做多次运行取均值，并给出置信区间。
- **元胞自动机结果需敏感性分析**：对 CA 规则中的关键参数（如阈值、邻域定义）进行敏感性分析，展示不同参数下的演化结果，证明结论的稳健性。
- **马尔科夫链需验证马尔可夫性**：使用之前需通过统计检验（如卡方检验）验证序列是否满足马尔可夫性，必要时使用高阶马尔科夫链。
