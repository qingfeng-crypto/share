...
rng(42);
X = randn(100, 3);
X(1, 1) = nan; X(2, 2) = nan; X(3, 3) = nan;  % 引入缺失值
X(4, 1) = 10; X(5, 2) = -8;  % 引入异常值

% 转换为表格
T = array2table(X, 'VariableNames', {'Feature1', 'Feature2', 'Feature3'});
...
% 2. 异常值检测 — isoutlier
[~, outlierIdx] = isoutlier(X(:, 1));
fprintf('Feature1 异常值索引: %s\n', mat2str(find(outlierIdx)));

% 3. Z-score 标准化
Z = zscore(X, 'omitnan');
fprintf('Z-score 标准化后均值: %.4f\n', mean(Z, 'omitnan'));

% 4. Min-Max 归一化到 [0, 1]
R = rescale(X, 0, 1);
fprintf('Min-Max 归一化后范围: [%.4f, %.4f]\n', min(R(:)), max(R(:)));
```
...
% 生成示例数据: 3个簇
rng(42);
X = [randn(50,2) + [2 2]; randn(50,2) + [-2 -2]; randn(50,2) + [0 -4]];

% 1. K-Means 聚类
[idx_kmeans, C] = kmeans(X, 3, 'Replicates', 10);
fprintf('K-Means 聚类完成\n');
...
% 3. DBSCAN 聚类
idx_dbscan = dbscan(X, 1.5, 5);
fprintf('DBSCAN 聚类完成, 簇数: %d\n', length(unique(idx_dbscan)) - 1);
```
...
% 1. 单样本 t 检验 (H0: 均值 = 0)
[h1, p1, ci1, stats1] = ttest(x, 0);
fprintf('单样本 t 检验: h=%d, p=%.4f\n', h1, p1);

% 2. 两样本方差齐性检验
[h2, p2, stats2] = vartest2(x, y);
fprintf('方差齐性检验: h=%d, p=%.4f\n', h2, p2);

% 3. 单因素方差分析
group = [ones(20,1); 2*ones(20,1); 3*ones(20,1)];
data = [randn(20,1)+1; randn(20,1)+2; randn(20,1)+3];
[p3, tbl, stats3] = anova1(data, group, 'off');
fprintf('单因素方差分析: p=%.4f\n', p3);
```
...
rng(42);
X = randn(100, 5);
X = X * diag([10, 5, 2, 1, 0.5]);  % 不同尺度的特征

% PCA
[coeff, score, latent, tsquared, explained] = pca(X);

fprintf('各主成分解释方差比: %s\n', mat2str(explained', 3));
fprintf('前2个主成分累计解释方差: %.2f%%\n', sum(explained(1:2)));
...
| 算法 | 时间复杂度 | 空间复杂度 | 说明 |
|------|-----------|-----------|------|
| 数据预处理（缺失值/异常值） | O(n·m) | O(n·m) | n 为样本数，m 为特征数 |
| K-Means 聚类 | O(I·n·k·m) | O(n·m) | I 为迭代次数，k 为簇数 |
| DBSCAN 聚类 | O(n²) 最坏, O(n·log n) 优化 | O(n) | 最坏为全连接数据 |
| 层次聚类 | O(n³) 最坏 | O(n²) | 距离矩阵计算 |
| 假设检验（t 检验/卡方检验） | O(n) | O(n) | 线性复杂度 |
| PCA 主成分分析 | O(n·m² + m³) | O(m²) | 协方差矩阵计算 |
| 因子分析 | O(n·m² + m³) | O(m²) | 与 PCA 同阶 |
| 典型相关分析 CCA | O(n·(p+q)² + (p+q)³) | O((p+q)²) | p,q 为两组变量维数 |
| NMF 非负矩阵分解 | O(I·n·k·m) | O(n·k + m·k) | I 为迭代次数 |

注：复杂度由算法本身决定，Python 与 MATLAB 实现共享同一复杂度。

### 局限性

- **K-Means 需预先指定 K 值**：聚类数 K 需人工设定，不同 K 值结果差异大。可用肘部法则或轮廓系数辅助确定，但无绝对标准。
- **PCA 损失可解释性**：主成分是原始特征的线性组合，物理含义不明确，降维后的特征难以直接用于解释性分析。
- **DBSCAN 对密度差异敏感**：全局 ε 参数难以适应不同密度区域，数据集中存在密度差异大的簇时聚类效果差。
- **层次聚类不可伸缩**：O(n²) 或 O(n³) 的复杂度使其无法处理大规模数据集（n > 10000）。
- **因子分析对数据要求高**：需要变量间存在较强相关性，且要求多元正态分布假设，偏离时结果不稳定。
- **NMF 分解结果不唯一**：非负矩阵分解存在多解性，不同初始化可能导致不同的分解结果，需多次运行取稳定解。

### 比赛使用建议

- **先可视化再选方法**：在建模前对数据进行可视化（散点图、箱线图、分布图），直观了解数据分布特征，据此选择合适的方法。
- **多方法交叉验证**：聚类分析中同时使用 K-Means 和 DBSCAN，对比结果并分析差异原因；降维分析中同时使用 PCA 和 t-SNE 进行对比。
- **数据预处理步骤需详细说明**：缺失值处理方法、异常值检测阈值、标准化方式等预处理细节必须在论文中说明，因为这些选择直接影响后续分析结果。
- **假设检验需明确前提条件**：t 检验要求正态性和方差齐性，卡方检验要求期望频数不低于 5，检验前需进行前提条件验证。
- **降维结果需配合可视化**：PCA 或因子分析结果用碎石图、载荷图、得分图可视化展示，帮助读者直观理解降维效果。
